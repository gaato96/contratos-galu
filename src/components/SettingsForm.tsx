...
import { getAgencySettingsAction, saveAgencySettingsAction } from "@/app/actions/settings";
...
export default function SettingsForm() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await getAgencySettingsAction();
                if (data) {
                    setClauses(data.default_clauses || []);
                    setLogoBase64(data.logo_base64 || null);
                    setSignatureBase64(data.agency_signature_base64 || null);
